...
(effect_size_plot.AllDec2 <- model_plot_sig_function(coeff.shrub_gradient.AllDec2, title_string = "deciduous shrubs", plot_width = 10.5))
```
    ...
    # LIKELIHOOD for continuous part
      ...
      for (j in 1:N_cont){
        ...
        logit(mu2[j]) <- b_plotgroup[plotgroup.cont[j]] + # ~= random effect of plot group
                        ...
                        b.tempXcompet * tempjja.cont[i] * compet.cont[i] +       # for interaction
                        b.tempXcompet2 * (tempjja.cont[i]^2) * compet.cont[i] +  # for interaction
...
* **not converging**: large R-hat values (~ 1.2) (only 16 non-zero values across plot groups)
...
* *positive linear relationship to summer precipitation (m.s.)*
* slightly unimodal relationship with summer temperature and topographic wetness (n.s.)
...
* **not converged** large R-hat values (~ 1.2) (only 10 non-zero values across plot groups)
...
* **not converging**: relatively large R-hat values (~ 1.05), large variation in parameter estimates (only 13 non-zero values)
...
* **not converging well**: relatively low R-hat values (< 1.02), but quite some variation in trace plots, especially for climatic parameters (only 13 non-zero values across plot groups)
...
* **positive relationship with more conservative communities (sig.)** and *summer temperature (m.s.)*
