...
predictors_set <- env_cov_bio_sub %>% 
  select(ends_with("_ts_30"),   # CHELSA predictors averaged over 10-year period prior to study year
         inclin_down, sri, tri, twi_90m,      # environmental data
         compet) %>% 
  names()
...
shrub_gradient_jags.BetNan.data <- list(
  ...
  xhat_twi = seq(from = min(BetNan.tot$twi_90mC), to = max(BetNan.tot$twi_90mC), length.out = 100),
...
shrub_gradient_jags.CasTet.data <- list(
  ...
  xhat_twi = seq(from = min(CasTet.tot$twi_90mC), to = max(CasTet.tot$twi_90mC), length.out = 100),
...
shrub_gradient_jags.EmpNig.data <- list(
  ...
  xhat_twi = seq(from = min(EmpNig.tot$twi_90mC), to = max(EmpNig.tot$twi_90mC), length.out = 100),
...
shrub_gradient_jags.PhyCae.data <- list(
  ...
  xhat_twi = seq(from = min(PhyCae.tot$twi_90mC), to = max(PhyCae.tot$twi_90mC), length.out = 100),
...
shrub_gradient_jags.RhoGro.data <- list(
  ...
  xhat_twi = seq(from = min(RhoGro.tot$twi_90mC), to = max(RhoGro.tot$twi_90mC), length.out = 100),
...
shrub_gradient_jags.RhoTom.data <- list(
  ...
  xhat_twi = seq(from = min(RhoTom.tot$twi_90mC), to = max(RhoTom.tot$twi_90mC), length.out = 100),
...
shrub_gradient_jags.SalArc.data <- list(
  ...
  xhat_twi = seq(from = min(SalArc.tot$twi_90mC), to = max(SalArc.tot$twi_90mC), length.out = 100),
...
shrub_gradient_jags.SalGla.data <- list(
  ...
  xhat_twi = seq(from = min(SalGla.tot$twi_90mC), to = max(SalGla.tot$twi_90mC), length.out = 100),
...
shrub_gradient_jags.VacUli.data <- list(
  ...
  xhat_twi = seq(from = min(VacUli.tot$twi_90mC), to = max(VacUli.tot$twi_90mC), length.out = 100),
...
(effect_size_plot.BetNan <- model_plot_sig_function(coeff.shrub_gradient.BetNan, title_string = "Betula nana", plot_width = 11))
```
...
# effect size plot
(effect_size_plot.EmpNig <- model_plot_marg_function(coeff.shrub_gradient.EmpNig, title_string = "Empetrum nigrum", plot_width = 11))
...
* **unimodal response to temperature & precipitation (linear, sig.)** 
...
# effect size plot
(effect_size_plot.SalGla2 <- model_plot_sig_function(coeff.shrub_gradient.SalGla2, title_string = "Salix glauca", plot_width = 8))
...
unique.tempcont <- seq(min(BetNan.tot$tempcont_ts_30C), max(BetNan.tot$tempcont_ts_30C), by=0.02) 
...
predsmatrix.tempcont.BetNan <- data.frame(expand.grid(tempcont_ts_30C=c(unique.tempcont)))
...
predsmatrix.tempcont.BetNan$tempcont <- attr(scale(BetNan.tot$tempcont_ts_30), 'scaled:scale') + attr(scale(BetNan.tot$tempcont_ts_30), 'scaled:center')

ggplot() +
  # tempcont is modelled at plotgroup level, so reduce base data (points layer) to plotgroup level
  geom_point(data = BetNan.tot %>% group_by(site_alt_plotgroup_id) %>% summarise(tempcont = mean(tempcont_ts_30), cover = mean(cover)), 
             aes(x = tempcont, 
                 y = cover), 
             size = 2,
