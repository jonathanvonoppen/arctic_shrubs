...
phats <- coeff.shrub_gradient.BetNan2 %>% 
  ...
  # add xhats column
  mutate(xhats = seq(from = min(BetNan.tot$tempcont_ts_30C),
                     to = max(BetNan.tot$tempcont_ts_30C),
                     length.out = 100))

# back-center and back-scale predictor values (xhats)
phats$tempcont <- phats$xhats*attr(scale(BetNan.tot$tempcont_ts_30), 'scaled:scale') + attr(scale(BetNan.tot$tempcont_ts_30), 'scaled:center') 
