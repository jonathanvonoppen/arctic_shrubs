...
* cover (as relative no. hits per plot)
```{r}
...
Let's check for correlation between the different moisture predictors:

```{r}
(cor_moist <- env_cov_bio %>% 
  select(twi_90m, ndwi, tcws)) %>% 
  correlate(diagonal = 1)
```
$\Rightarrow$ NDWI and TCWS are highly correlated, but both are largely independent from TWI.
We'll go with TWI for now, but will have to discuss maybe including a second index, depending on hypothesis/ecological implication.
...
predictors_set <- env_cov_bio_sub %>% 
  select(ends_with("_ts_30"),   # CHELSA predictors averaged over 10-year period prior to study year
         inclin_down, twi_90m, sri,    # environmental data
         compet) %>% 
  names()
...
(vif_predictors_1 <- env_cov_bio_sub %>% 
  dplyr::select(predictors_set) %>% 
  usdm::vif()) #%>% View()
# => drop tempmax, tempmin, inclin_down
(vif_predictors_2 <- env_cov_bio_sub %>% 
  dplyr::select(predictors_set,
                ...
                -inclin_down) %>% 
  usdm::vif()) #%>% View()
# => drop winter & spring temperature
(vif_predictors_3 <- env_cov_bio_sub %>% 
  dplyr::select(predictors_set,
                ...
                -contains("mam")) %>% 
  usdm::vif()) #%>% View()
# all VIF < 2.6 -> OK! >> exclude dropped variables from final dataset:
...
write("
  ...
  ","../scripts/jonathan/shrub_gradient.spec.jags")
```
...
...and can run the model
```{r}
# run model
model_out.shrub_gradient.BetNan <- jags(shrub_gradient_jags.BetNan.data, inits = NULL, params, 
                                 model.file = "../scripts/jonathan/shrub_gradient.spec.jags", n.chains = 3, 
                                 n.iter = 8000, n.burnin = 6000, n.thin=2, DIC=FALSE, 
                                 working.directory=NULL, progress.bar = "text") 
...
```{r}
model_out.shrub_gradient.CasTet <- jags(shrub_gradient_jags.CasTet.data, inits = NULL, params, 
                                 model.file = "../scripts/jonathan/shrub_gradient.spec.jags", n.chains = 3, 
                                 n.iter = 8000, n.burnin = 6000, n.thin=2, DIC=FALSE, 
                                 working.directory=NULL, progress.bar = "text") 
...
```{r}
model_out.shrub_gradient.EmpNig <- jags(shrub_gradient_jags.EmpNig.data, inits = NULL, params, 
                                 model.file = "../scripts/jonathan/shrub_gradient.spec.jags", n.chains = 3, 
                                 n.iter = 8000, n.burnin = 6000, n.thin=2, DIC=FALSE, 
                                 working.directory=NULL, progress.bar = "text") 
...
```{r}
model_out.shrub_gradient.PhyCae <- jags(shrub_gradient_jags.PhyCae.data, inits = NULL, params, 
                                 model.file = "../scripts/jonathan/shrub_gradient.spec.jags", n.chains = 3, 
                                 n.iter = 8000, n.burnin = 6000, n.thin=2, DIC=FALSE, 
                                 working.directory=NULL, progress.bar = "text") 
...
```{r}
model_out.shrub_gradient.RhoGro <- jags(shrub_gradient_jags.RhoGro.data, inits = NULL, params, 
                                 model.file = "../scripts/jonathan/shrub_gradient.spec.jags", n.chains = 3, 
                                 n.iter = 8000, n.burnin = 6000, n.thin=2, DIC=FALSE, 
                                 working.directory=NULL, progress.bar = "text") 
...
```{r}
model_out.shrub_gradient.RhoTom <- jags(shrub_gradient_jags.RhoTom.data, inits = NULL, params, 
                                 model.file = "../scripts/jonathan/shrub_gradient.spec.jags", n.chains = 3, 
                                 n.iter = 8000, n.burnin = 6000, n.thin=2, DIC=FALSE, 
                                 working.directory=NULL, progress.bar = "text") 
...
```{r}
model_out.shrub_gradient.SalArc <- jags(shrub_gradient_jags.SalArc.data, inits = NULL, params, 
                                 model.file = "../scripts/jonathan/shrub_gradient.spec.jags", n.chains = 3, 
                                 n.iter = 8000, n.burnin = 6000, n.thin=2, DIC=FALSE, 
                                 working.directory=NULL, progress.bar = "text") 
...
```{r}
model_out.shrub_gradient.SalGla <- jags(shrub_gradient_jags.SalGla.data, inits = NULL, params, 
                                 model.file = "../scripts/jonathan/shrub_gradient.spec.jags", n.chains = 3, 
                                 n.iter = 8000, n.burnin = 6000, n.thin=2, DIC=FALSE, 
                                 working.directory=NULL, progress.bar = "text") 
...
```{r}
model_out.shrub_gradient.VacUli <- jags(shrub_gradient_jags.VacUli.data, inits = NULL, params, 
                                 model.file = "../scripts/jonathan/shrub_gradient.spec.jags", n.chains = 3, 
                                 n.iter = 8000, n.burnin = 6000, n.thin=2, DIC=FALSE, 
                                 working.directory=NULL, progress.bar = "text") 
