...
model_out.shrub_gradient.BetNan <- jags(shrub_gradient_jags.BetNan.data,    # input data
                                        ...
                                        n.chains = 3,                       # no. Markov chains
                                        n.iter = 100000, n.burnin = 80000,     # no. iterations & burn-in fraction per chain
                                        n.thin = 2,                         # thinning rate
                                        DIC = FALSE,                        # do not compute deviance, pD, and DIC
                                        working.directory = NULL, 
                                        progress.bar = "text") 
...
model_plot_function <- function(model_coeff_output) {
  ...
  names(solutions) <- c("post.mean", "post.sd", "l95", "u95", "Rhat", "variable")
  solutions <- solutions %>% 
...
(coeff.shrub_gradient.CasTet)
model_plot_function(coeff.shrub_gradient.CasTet)
```
...
(coeff.shrub_gradient.EmpNig)

# effect size plot
model_plot_function(coeff.shrub_gradient.EmpNig)

```

* **positive response to precipitation (linear, sig.)**
* negative response to temperature (quadratic, n.s.)
...
(coeff.shrub_gradient.RhoGro)

# effect size plot
model_plot_function(coeff.shrub_gradient.RhoGro)

```

* **negative response to precipitation (linear, sig.) & temperature (quadratic, strongly sig.)**
* slightly positive response to SRI, temp. variation (n.s.)
* slightly negative responst to TWI (n.s.)
* large variation in competiton response as no taller-growing species in the system
<br>
...
(coeff.shrub_gradient.SalGla)

# effect size plot
model_plot_function(coeff.shrub_gradient.SalGla)
...
(coeff.shrub_gradient.VacUli)

# effect size plot
model_plot_function(coeff.shrub_gradient.VacUli)

```

* **negative response to temperature (quadratic, sig.)**, competition, & TWI (n.s.)
