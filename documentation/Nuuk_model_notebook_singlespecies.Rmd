...
model_plot_sig_function <- function(model_coeff_output, title_string) {
...
model_plot_marg_function <- function(model_coeff_output, title_string) {
  target_vars <- c("b.tempjja.x", "b.tempjja.x2",
                   "b.tempcont.x", "b.tempcont.x2",
                   "b.precipjja.x", "b.precipjja.x2",
                   "b.sri",
...
How do the trends in cover response look for each of the predictors? Plot panels:
```{r}
...
(effect_size_plot.BetNan <- model_plot_sig_function(coeff.shrub_gradient.BetNan, title_string = "Betula nana"))
```

* **negative response to temperature variability (sig)**
...
# effect size plot
(effect_size_plot.EmpNig <- model_plot_marg_function(coeff.shrub_gradient.EmpNig, title_string = "Empetrum nigrum"))

```

* **positive response to precipitation (linear, sig.)**
* *negative response to temperature (quadratic, m.s.)*
...
# effect size plot
(effect_size_plot.RhoGro <- model_plot_sig_function(coeff.shrub_gradient.RhoGro, title_string = "Rhododendron groenlandicum"))

```

* **negative response to temperature (quadratic, strongly sig.) & precipitation (linear, sig.)** 
* slightly positive response to SRI, temp. variation (n.s.)
* slightly negative responst to TWI (n.s.)
* large variation in competiton response as there were no taller-growing species in the system
<br>
...
# effect size plot
(effect_size_plot.SalGla <- model_plot_sig_function(coeff.shrub_gradient.SalGla, title_string = "Salix glauca"))
...
# effect size plot
(effect_size_plot.VacUli <- model_plot_marg_function(coeff.shrub_gradient.VacUli, title_string = "Vaccinium uliginosum"))

```

* **negative response to temperature (quadratic, sig.)**, *TWI, competition (m.s.)*
...
save_plot(file.path("..", "figures", "nuuk_shrub_drivers_betabinom_effect_size_panels_hor.eps"),
          nuuk_effectsize_plot_grid_hor, base_height = 10, base_aspect_ratio = 1)
```

## Save workspace image
```{r}
save.image(file = file.path("..", "data", "processed", "nuuk_shrub_drivers_output.RData"))

# load ws for further analyses/plotting etc)
# load(file.path("..", "data", "processed", "nuuk_shrub_drivers_output.RData"))
```
