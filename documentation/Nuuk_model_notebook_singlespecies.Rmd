...
predictors_set <- env_cov_bio_sub %>% 
  select(ends_with("_ts_30"),   # CHELSA predictors averaged over 10-year period prior to study year
         ...
         ends_with("_cover"), compet) %>%  # biotic predictors
  names()
...
shrub_gradient_jags.AllShr.data <- list(
  ...
  tempjja.tot = AllShr.tot$tempjjaC[!duplicated(AllShr.tot$plotgroup.NUM)], # one value per tXpg
  tempcont.tot = AllShr.tot$tempcontC[!duplicated(AllShr.tot$plotgroup.NUM)],
  precipjja.tot = AllShr.tot$precipjjaC[!duplicated(AllShr.tot$plotgroup.NUM)],
...
shrub_gradient_jags.AllEve.data <- list(
  ...
  # plot group level predictors
  tempjja.tot = AllEve.tot$tempjjaC[!duplicated(AllEve.tot$plotgroup.NUM)], # one value per tXpg
  tempcont.tot = AllEve.tot$tempcontC[!duplicated(AllEve.tot$plotgroup.NUM)],
  precipjja.tot = AllEve.tot$precipjjaC[!duplicated(AllEve.tot$plotgroup.NUM)],
...
shrub_gradient_jags.AllDec.data <- list(
  ...
  # plot group level predictors
  tempjja.tot = AllDec.tot$tempjjaC[!duplicated(AllDec.tot$plotgroup.NUM)], # one value per tXpg
  tempcont.tot = AllDec.tot$tempcontC[!duplicated(AllDec.tot$plotgroup.NUM)],
  precipjja.tot = AllDec.tot$precipjjaC[!duplicated(AllDec.tot$plotgroup.NUM)],
...
write("
  ...
  model{
    ...
    # priors
      ...
      b.tempXtcws2a ~ dnorm(0, 0.001)
      b.tempXtcws2b ~ dnorm(0, 0.001)
      b.tempXtcws2c ~ dnorm(0, 0.001)
      b.tempXtcws3 ~ dnorm(0, 0.001)
    ...
    # LIKELIHOOD
      ...
      for (i in 1:N_plots){ 
        cov.tot[i] ~ dlnorm(mu[i])
        log(mu[i]) <- b_plotgroup[plotgroup.tot[i]] + # ~= random effect of plot group
      ...
      for (m in 1:Nxhat){
        ...
        for (p in 1:Nxhat2){
          phat_tempXmoist[m,p] <- intercept +
                                      ...
                                      b.tcws.x * xhat_tcws2[p] +
                                      b.tempXtcws2a * xhat_tempjja[m] * (xhat_tempjja[m]^2) +
                                      b.tempXtcws2b * xhat_tempjja[m] * xhat_tcws2[p] +
                                      b.tempXtcws2c * (xhat_tempjja[m]^2) * xhat_tcws2[p] +
                                      b.tempXtcws3 * xhat_tempjja[m] * (xhat_tempjja[m]^2) * xhat_tcws2[p]
        }
...
(effect_size_plot.AllShr <- model_plot_marg_function(coeff.shrub_gradient.AllShr, title_string = "all shrubs", plot_width = 11))
```
...
(effect_size_plot.AllEve <- model_plot_marg_function(coeff.shrub_gradient.AllEve, title_string = "evergreen shrubs", plot_width = 11))
```
...
shrub_gradient_jags.BetNan.data <- list(
  ...
  shrub_cov.dis = BetNan.dis$shrub_coverC,
  ...
  shrub_cov.cont = BetNan.cont$shrub_coverC,
  gramin_cov.cont = BetNan.cont$graminoid_coverC,
  compet.cont = BetNan.cont$competC,
  N_cont = nrow(BetNan.cont),
  
  # plot group level predictors
  tempjja.tot = BetNan.tot$tempjjaC[!duplicated(BetNan.tot$plotgroup.NUM)], # one value per tXpg
  tempcont.tot = BetNan.tot$tempcontC[!duplicated(BetNan.tot$plotgroup.NUM)],
  ...
  xhat_shrub_cover = seq(from = min(BetNan.tot$shrub_coverC), to = max(BetNan.tot$shrub_coverC), length.out = 100),
...
shrub_gradient_jags.CasTet.data <- list(
  ...
  shrub_cov.dis = CasTet.dis$shrub_coverC,
  ...
  shrub_cov.cont = CasTet.cont$shrub_coverC,
  gramin_cov.cont = CasTet.cont$graminoid_coverC,
  compet.cont = CasTet.cont$competC,
  N_cont = nrow(CasTet.cont),
  
  # plot group level predictors
  tempjja.tot = CasTet.tot$tempjjaC[!duplicated(CasTet.tot$plotgroup.NUM)], # one value per tXpg
  tempcont.tot = CasTet.tot$tempcontC[!duplicated(CasTet.tot$plotgroup.NUM)],
  precipjja.tot = CasTet.tot$precipjjaC[!duplicated(CasTet.tot$plotgroup.NUM)],
...
shrub_gradient_jags.EmpNig.data <- list(
  ...
  shrub_cov.dis = EmpNig.dis$shrub_coverC,
  ...
  shrub_cov.cont = EmpNig.cont$shrub_coverC,
  gramin_cov.cont = EmpNig.cont$graminoid_coverC,
  compet.cont = EmpNig.cont$competC,
  N_cont = nrow(EmpNig.cont),
  
  # plot group level predictors
  tempjja.tot = EmpNig.tot$tempjjaC[!duplicated(EmpNig.tot$plotgroup.NUM)], # one value per tXpg
  tempcont.tot = EmpNig.tot$tempcontC[!duplicated(EmpNig.tot$plotgroup.NUM)],
  precipjja.tot = EmpNig.tot$precipjjaC[!duplicated(EmpNig.tot$plotgroup.NUM)],
  ...
  xhat_shrub_cover = seq(from = min(EmpNig.tot$shrub_coverC), to = max(EmpNig.tot$shrub_coverC), length.out = 100),
...
shrub_gradient_jags.PhyCae.data <- list(
  ...
  shrub_cov.dis = PhyCae.dis$shrub_coverC,
  ...
  shrub_cov.cont = PhyCae.cont$shrub_coverC,
  gramin_cov.cont = PhyCae.cont$graminoid_coverC,
  compet.cont = PhyCae.cont$competC,
  N_cont = nrow(PhyCae.cont),
  
  # plot group level predictors
  tempjja.tot = PhyCae.tot$tempjjaC[!duplicated(PhyCae.tot$plotgroup.NUM)], # one value per tXpg
  tempcont.tot = PhyCae.tot$tempcontC[!duplicated(PhyCae.tot$plotgroup.NUM)],
  precipjja.tot = PhyCae.tot$precipjjaC[!duplicated(PhyCae.tot$plotgroup.NUM)],
  ...
  xhat_shrub_cover = seq(from = min(PhyCae.tot$shrub_coverC), to = max(PhyCae.tot$shrub_coverC), length.out = 100),
...
shrub_gradient_jags.RhoGro.data <- list(
  ...
  shrub_cov.dis = RhoGro.dis$shrub_coverC,
  ...
  shrub_cov.cont = RhoGro.cont$shrub_coverC,
  gramin_cov.cont = RhoGro.cont$graminoid_coverC,
  compet.cont = RhoGro.cont$competC,
  N_cont = nrow(RhoGro.cont),
  
  # plot group level predictors
  tempjja.tot = RhoGro.tot$tempjjaC[!duplicated(RhoGro.tot$plotgroup.NUM)], # one value per tXpg
  tempcont.tot = RhoGro.tot$tempcontC[!duplicated(RhoGro.tot$plotgroup.NUM)],
  precipjja.tot = RhoGro.tot$precipjjaC[!duplicated(RhoGro.tot$plotgroup.NUM)],
  ...
  xhat_shrub_cover = seq(from = min(RhoGro.tot$shrub_coverC), to = max(RhoGro.tot$shrub_coverC), length.out = 100),
...
shrub_gradient_jags.RhoTom.data <- list(
  ...
  shrub_cov.dis = RhoTom.dis$shrub_coverC,
  ...
  shrub_cov.cont = RhoTom.cont$shrub_coverC,
  gramin_cov.cont = RhoTom.cont$graminoid_coverC,
  compet.cont = RhoTom.cont$competC,
  N_cont = nrow(RhoTom.cont),
  
  # plot group level predictors
  tempjja.tot = RhoTom.tot$tempjjaC[!duplicated(RhoTom.tot$plotgroup.NUM)], # one value per tXpg
  tempcont.tot = RhoTom.tot$tempcontC[!duplicated(RhoTom.tot$plotgroup.NUM)],
  precipjja.tot = RhoTom.tot$precipjjaC[!duplicated(RhoTom.tot$plotgroup.NUM)],
  ...
  xhat_shrub_cover = seq(from = min(RhoTom.tot$shrub_coverC), to = max(RhoTom.tot$shrub_coverC), length.out = 100),
...
shrub_gradient_jags.SalArc.data <- list(
  ...
  shrub_cov.dis = SalArc.dis$shrub_coverC,
  ...
  shrub_cov.cont = SalArc.cont$shrub_coverC,
  gramin_cov.cont = SalArc.cont$graminoid_coverC,
  compet.cont = SalArc.cont$competC,
  N_cont = nrow(SalArc.cont),
  
  # plot group level predictors
  tempjja.tot = SalArc.tot$tempjjaC[!duplicated(SalArc.tot$plotgroup.NUM)], # one value per tXpg
  tempcont.tot = SalArc.tot$tempcontC[!duplicated(SalArc.tot$plotgroup.NUM)],
  precipjja.tot = SalArc.tot$precipjjaC[!duplicated(SalArc.tot$plotgroup.NUM)],
  ...
  xhat_shrub_cover = seq(from = min(SalArc.tot$shrub_coverC), to = max(SalArc.tot$shrub_coverC), length.out = 100),
...
shrub_gradient_jags.SalGla.data <- list(
  ...
  shrub_cov.dis = SalGla.dis$shrub_coverC,
  ...
  shrub_cov.cont = SalGla.cont$shrub_coverC,
  ...
  xhat_shrub_cover = seq(from = min(SaGla.tot$shrub_coverC), to = max(SaGla.tot$shrub_coverC), length.out = 100),
  xhat_graminoid_cover = seq(from = min(SaGla.tot$graminoid_coverC), to = max(SaGla.tot$graminoid_coverC), length.out = 100),
...
shrub_gradient_jags.VacUli.data <- list(
  ...
  shrub_cov.dis = VacUli.dis$shrub_coverC,
  ...
  shrub_cov.cont = VacUli.cont$shrub_coverC,
  gramin_cov.cont = VacUli.cont$graminoid_coverC,
  compet.cont = VacUli.cont$competC,
  N_cont = nrow(VacUli.cont),
  
  # plot group level predictors
  tempjja.tot = VacUli.tot$tempjjaC[!duplicated(VacUli.tot$plotgroup.NUM)], # one value per tXpg
  tempcont.tot = VacUli.tot$tempcontC[!duplicated(VacUli.tot$plotgroup.NUM)],
  precipjja.tot = VacUli.tot$precipjjaC[!duplicated(VacUli.tot$plotgroup.NUM)],
  ...
  xhat_shrub_cover = seq(from = min(VacUli.tot$shrub_coverC), to = max(VacUli.tot$shrub_coverC), length.out = 100),
  ...
  model{
    ...
    # LIKELIHOOD for continuous part
      ...
      for (m in 1:Nxhat){
        ...
        phat_graminoid_cover[m] <- intercept + b.gramin_cov * xhat_graminoid_cover[m]
        phat_shrub_cover[m] <- intercept + b.shrub_cover * xhat_shrub_cover[m]
        phat_sri[m] <- intercept + b.sri * xhat_sri[m]
        phat_tri[m] <- intercept + b.tri * xhat_tri[m]
        phat_tcws[m] <- intercept + b.tcws * xhat_tcws[m]
        phat_tempjja[m] <- intercept + b.tempjja.x * xhat_tempjja[m] + b.tempjja.x2 * (xhat_tempjja[m]^2)
...
write("
  ...
  model{
    ...
    # LIKELIHOOD for continuous part
      ...
      # add predicted values (derived parameters)
      for (m in 1:Nxhat){
        phat_compet[m] <- intercept + 
                          b.compet * xhat_compet[m]
        phat_graminoid_cover[m] <- intercept + 
                                    b.gramin_cov * xhat_graminoid_cover[m]
        phat_shrub_cover[m] <- intercept + 
                                b.shrub_cover * xhat_shrub_cover[m]
        phat_sri[m] <- intercept + 
                        b.sri * xhat_sri[m]
        phat_tri[m] <- intercept + 
                        b.tri * xhat_tri[m]
        phat_tcws[m] <- intercept + 
                        b.tcws * xhat_tcws[m]
