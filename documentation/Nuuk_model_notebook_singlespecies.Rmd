...
predsmatrix.tempcont.BetNan$tempcont <- predsmatrix.tempcont.BetNan$tempcont_ts_30C + attr(scale(BetNan.tot$tempcont_ts_30), 'scaled:center')
...
shrub_gradient_jags.SalGla.xhat.data <- list(
  ...
  xhat = seq(from = min(SalGla.tot$compet), to = max(SalGla.tot$compet), length.out = 100),
  Nxhat = 100
  # # site/alt level predictors
  # alt.tot = SalGla.tot$altC[!duplicated(SalGla.tot$site_alt.NUM)],
  # N_isoclines = length(unique(SalGla.tot$site_alt_id))
...
write("
  ...
  model{
    ...
    # LIKELIHOOD for continuous part
      ...
      for (m in 1:Nxhat){
        phat[m] <- intercept + b.compet * xhat[m]
      }
