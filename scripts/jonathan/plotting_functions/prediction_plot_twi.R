...
prediction_plots_species <- function(species) {
  ...
  phats_long <- phats_long %>% 
    ...
    mutate(pred_id = factor(str_remove(str_remove(param, "phat_"), "\\[\\d+\\]"),
                            levels = c("tempjja", "tempcont", "precipjja", "sri", "tri", "twi", "compet", "shrub_cover", "graminoid_cover"))) %>% 
    
    # add significance level column
    mutate(sig = "ns") 
  
  param_lookup <- data.frame(coeffs = c("b.tempjja.x", "b.tempjja.x2", "b.tempcont.x", "b.tempcont.x2", "b.precipjja.x", "b.precipjja.x2", "b.sri", "b.tri", "b.twi", "b.compet", "b.shrub_cov", "b.gramin_cov"),
                             phats = c("tempjja", "tempjja", "tempcont", "tempcont", "precipjja", "precipjja", "sri", "tri", "twi", "compet", "shrub_cover", "graminoid_cover"))
...
prediction_plots_groups <- function(fgroup) {
  ...
  predictor_phats <- c("phat_sri", "phat_tri", "phat_twi", 
                       "phat_tempjja", "phat_tempcont", "phat_precipjja", 
  ...
  phats_long <- phats_long %>% 
    ...
    # extract predictor strings from param column
    mutate(pred_id = factor(str_remove(str_remove(param, "phat_"), "\\[\\d+\\]"),
                            levels = c("tempjja", "tempcont", "precipjja", "sri", "tri", "twi", "graminoid_cover"))) %>% 
