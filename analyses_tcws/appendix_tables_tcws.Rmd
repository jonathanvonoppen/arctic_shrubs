---
title: Variables explaining tundra shrub abundance in SW Greenland - supplementary
  material
...
output:
  ...
  word_document:
    toc: yes
    toc_depth: '3'
subtitle: supporting information to Appendix S7 (analyses incl. TCWS)
---
...
(compare Appendix S4, Table S4.1, for TWI)
...
(compare Appendix S5, Table S5.1, for TWI)
...
## Model output tables (TCWS, final models) 

(compare Appendix S6, Tables S6.1 - S6.12, for TWI)
...
source(file = file.path("..", "scripts", "JvO", "plotting_functions", "output_tables.R"))
```
...
## Model output tables (TCWS, full models for focal groups and species) 

(including all quadratic terms for climatic variables that were removed in final models)

```{r, echo = FALSE}
# load model outputs
# species
model_outputs_species <- file.path("..", "data", "processed", "model_outputs", "species_tcws", "first_runs_converged", list.files(path = file.path("..", "data", "processed", "model_outputs", "species_tcws", "first_runs_converged"), pattern = "*.Rdata"))
for (model_output in model_outputs_species){
  load(model_output)
}
# groups
model_outputs_groups <- file.path("..", "data", "processed", "model_outputs", "groups_tcws", "first_runs_converged", list.files(path = file.path("..", "data", "processed", "model_outputs", "groups_tcws", "first_runs_converged"), pattern = "*.Rdata"))
for (model_output in model_outputs_groups){
  load(model_output)
}
...
```{r, echo = FALSE}
# design tables
(AllShr_full_output_table <- output_table_gr(fgroup = "all shrubs"))
```
\newpage
...
```{r, echo = FALSE}
# design tables
(AllEve_full_output_table <- output_table_gr(fgroup = "evergreen shrubs"))
```
\newpage
...
```{r, echo = FALSE}
# design tables
(AllDec_full_output_table <- output_table_gr(fgroup = "deciduous shrubs"))
```
\newpage
...
```{r, echo = FALSE}
# design tables
(BetNan_full_output_table <- output_table_sp(species = "Betula nana"))
```
\newpage
...
```{r, echo = FALSE}
# design tables
(EmpNigfull__output_table <- output_table_sp(species = "Empetrum nigrum"))
```
\newpage
...
```{r, echo = FALSE}
# design tables
(RhoGro_full_output_table <- output_table_sp(species = "Rhododendron groenlandicum"))
```
\newpage
...
```{r, echo = FALSE}
# design tables
(SalGla_full_output_table <- output_table_sp(species = "Salix glauca"))
```
\newpage
...
```{r, echo = FALSE}
# design tables
(VacUli_full_output_table <- output_table_sp(species = "Vaccinium uliginosum"))
```
